...
export const CardComponent = ({flag,name,capital,handleVisitedCountry}) => {
...
const handleIsVisited=(name)=>{
  ...
  return (
   <div className='border-8 rounded-xl border-black bg-[#282727] p-2'>
    ...
    <button onClick={()=>handleIsVisited(name)} className={` ${isVisited? "bg-green-500":"bg-red-500"} rounded-xl text-white px-2 py-1 `}>{isVisited? "Visited":"Visited?"}</button>
    </div>
   </div>
  )
}
